  ...
  return <main>
    ...
    <div className="layout"><div className="maincol">
      ...
      <Panel title="Actual MT5 positions">
        <table><thead><tr><th>Ticket</th><th>Side</th><th>Volume</th><th>Open</th><th>P&amp;L</th></tr></thead><tbody>{positions.map(p => <tr key={p.ticket}><td>{p.ticket}</td><td>{p.side}</td><td>{p.volume}</td><td>{p.price_open}</td><td>{p.profit.toFixed(2)}</td></tr>)}</tbody></table>
