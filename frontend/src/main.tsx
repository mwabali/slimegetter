  ...
  return <main>
    ...
    <div className="layout"><div className="maincol">
      ...
      <Panel title="Actual MT5 positions">
        <table><thead><tr><th>Ticket</th><th>Side</th><th>Open</th><th>P&amp;L</th><th>R</th><th>Peak</th><th>Policy</th><th>State</th><th>Floor</th><th>Attempts</th><th>Issue</th></tr></thead><tbody>{positions.map(p => <tr key={p.ticket}><td>{p.ticket}</td><td>{p.side}</td><td>{p.price_open}</td><td>{p.profit.toFixed(2)}</td><td>{p.current_r?.toFixed(2) ?? "—"}</td><td>{p.peak_profit?.toFixed(2) ?? "—"}</td><td>{p.active_exit_policy ?? "—"}</td><td><State value={p.profit_management_state ?? "MONITORING"}/></td><td>{p.trailing_floor?.toFixed(2) ?? p.locked_profit_floor?.toFixed(2) ?? "—"}</td><td>{p.close_attempt_count}</td><td>{p.execution_locked ? "LOCKED" : p.latest_mt5_error ? p.latest_mt5_error.slice(0, 42) : "—"}</td></tr>)}</tbody></table>
