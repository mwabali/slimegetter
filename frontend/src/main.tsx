...
function App() {
  ...
  const health = status ? Object.fromEntries(Object.entries({
    ...
    "Simulation worker": status.simulation_worker, Pixis: status.demo_position_manager,
  ...
  return <main>
    ...
    <div className="layout"><div className="maincol">
      ...
      <Panel title="Journal filters"><select value={agentFilter} onChange={e => setAgentFilter(e.target.value)}><option value="">All bots</option><option>ANNIE</option><option>MIKASA</option><option>EREN</option><option>COMMANDER_ERWIN</option><option>PIXIS</option><option>ARMIN</option><option>CPT_LEVI</option><option>SIMULATION</option></select><input value={correlationFilter} onChange={e => setCorrelationFilter(e.target.value)} placeholder="Correlation ID"/><button onClick={() => void load()}>Apply</button></Panel>
