...
## Levi Daily Research Reports

Levi reports are generated by a separate read-only reporting command. It reads
the journal database and writes sanitized summaries to `reports/daily/`; it does
not submit orders, close positions, modify SL/TP, modify strategy settings, or
resolve incidents.

Install the Windows scheduled task from the repo root:

```powershell
powershell -ExecutionPolicy Bypass -File .\backend\scripts\install_levi_daily_report_task.ps1 -Time 00:30
```

The GitHub workflow `.github/workflows/levi-daily-report.yml` publishes the same
report after the configured cutoff and commits only when report files changed.
